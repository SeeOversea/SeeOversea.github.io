  ...
  function createTooltip() {
    ...
    this.container.on('mousemove', function (event) {
      if (map.tooltip.selector.style.display === 'block') {
        ...
        var left=event.pageX+20, top=event.pageY-100;
        if (left > (window.innerWidth*0.75)) {
          left = event.pageX - 20 - (window.innerWidth*0.25);
        }

        if (top > (window.innerHeight*0.4)) {
          top = event.pageY - (window.innerHeight*0.6) + 100;
        }
